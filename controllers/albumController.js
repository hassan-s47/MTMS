...
const addSongView = async (req, res) => {
  Student.find({}, (err, items) => {
    if (err) {
      ...
      // console.log(items);
      res.render('addSongs', { id: req.params.id, items });
    }
  });
};
    ...
    .then((response) => {
      ...
      Album.find({ teacher: req.session.user }, (err, items) => {
        if (err) {
          ...
          res.render('album', { items, success: 'Album Created', failure: '' });
        }
      });
    })
    .catch((err) => {
      Album.find({ teacher: req.session.user }, (err, items) => {
        if (err) {
          ...
          // console.log(items);
          res.render('album', { items, success: 'Album Created', failure: 'Error! cannot Upload the album' });
        }
      });
    });
};
...
const getAlbum = async (req, res) => {
  Album.find({ teacher: req.session.user }, (err, items) => {
    if (err) {
      ...
      res.render('album', { items, success: '', failure: '' });
    }
  });
};
